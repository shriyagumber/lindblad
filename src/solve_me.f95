...
subroutine me(hamic, lindbladic, boltz, temperature, n_states, nelect, dtelec, n_files, nsteps, istate, gamma, den)
    ...
    real, parameter :: kb = .0000862   ! eV/K
    ...
    do istep = 1, total_steps-1
        ...
        call rk4_lindblad(rho, H, L, hbar, dtelec, n_states, rho_next)
...
subroutine rk4_lindblad(rho, H, L, hbar, dt, n, rho_out)
    implicit none
    integer, intent(in) :: n
    real, intent(in) :: dt, hbar
    complex, intent(in) :: rho(n,n), H(n,n), L(n,n)
    ...
    call compute_drho_lindblad(rho, H, L, hbar, n, k1)
    tmp = rho + 0.50 * dt * k1
    call compute_drho_lindblad(tmp, H, L, hbar, n, k2)
    tmp = rho + 0.50 * dt * k2
    call compute_drho_lindblad(tmp, H, L, hbar, n, k3)
    tmp = rho + dt * k3
    call compute_drho_lindblad(tmp, H, L, hbar, n, k4)
...
subroutine compute_drho_lindblad(rho, H, L, hbar, n, drho)
    implicit none
    integer, intent(in) :: n
    real, intent(in) :: hbar
    complex, intent(in) :: rho(n,n), H(n,n), L(n,n)
    ...
    ! Dissipator
    Ldag = transpose(conjg(L))
    LrhoLdag = matmul(L, matmul(rho, Ldag))
    LdagL = matmul(Ldag, L)
